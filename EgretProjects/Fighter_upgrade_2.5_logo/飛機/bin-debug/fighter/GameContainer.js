    ...
    var GameContainer = (function (_super) {
        ...
        GameContainer.prototype.touchHandler = function (evt) {
            if (evt.type == egret.TouchEvent.TOUCH_TAP) {
                var tx = evt.localX;
                var begin = this.myFighter.x;
                tx = Math.max(this.myFighter.width / 2, tx);
                tx = Math.min(this.stageW - this.myFighter.width / 2, tx);
                this.myFighter.x = tx;
                /*    this.myFighter.addEventListener(egret.Event.ENTER_FRAME,()=>{
                        this.myFighter.x = begin;
                        if(tx<this.myFighter.x){begin-=5;}
                        else if(tx>this.myFighter.x){begin+=5;}
                        else{this.myFighter.x=tx}
        ...
        GameContainer.prototype.loadSound = function () {
            ...
            sound.load("resource/assets/東方背景音樂.mp3");
        };
